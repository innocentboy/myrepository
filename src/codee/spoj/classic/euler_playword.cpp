    ...
    if(orderVar == 1){if(bStart)return -1; bStart=1;startCh=i;continue;}
    if(orderVar == -1){if(bEnd)return -1; bEnd=1;endCh=i;continue;}
    if(orderVar)return -1;
    }
    return 0;
    }
    ...
    if(startCh != -1)
    {
    for(i=0;i<26;i++){nodeCount+=nodeFlag[i];visited[i]=0;}
    i=0;
    while(!nodeFlag[i] && i<26)i++;
    visited[i]=1;
    NodeConnect(i,visited);
    for(i=0;i<26;i++){visitedCount+=visited[i];}
